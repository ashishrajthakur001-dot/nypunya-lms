...
async function query<T>(table: string, params = ''): Promise<T[]> {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${params}`, { headers: authHeaders(), cache: 'no-store' })
...
export async function getEnrollments(studentId?: string) {
  const id = studentId ?? (await getProfile())?.id
  if (!id) return []
  return query<Enrollment>('enrollments', `select=id,course_id,student_id,status,progress&student_id=eq.${id}&order=enrolled_at.desc`)
}
export async function getProfile() {
  const rows = await query<Profile>('profiles', 'select=id,full_name,role&limit=1')
  return rows[0] ?? null
}
